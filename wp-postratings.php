...
function the_ratings_header() {
	...
	wp_register_script('wp-postratings', plugins_url('wp-postratings/postratings-js.js'), false, '1.41');
	if(@file_exists(TEMPLATEPATH.'/postratings-css.css')) {
		wp_register_style('wp-postratings', get_stylesheet_directory_uri().'/postratings-css.css', false, '1.41', 'all');
	} else {
		wp_register_style('wp-postratings', plugins_url('wp-postratings/postratings-css.css'), false, '1.41', 'all');
	}	
	if('rtl' == $text_direction) {
		if(@file_exists(TEMPLATEPATH.'/postratings-css-rtl.css')) {
			wp_register_style('wp-postratings-rtl', get_stylesheet_directory_uri().'/postratings-css-rtl.css', false, '1.41', 'all');
		} else {
			wp_register_style('wp-postratings-rtl', plugins_url('wp-postratings/postratings-css-rtl.css'), false, '1.41', 'all');
		} 
	}
...
### Function: Display Rating Footer
add_action('wp_footer', 'the_ratings_footer');
function the_ratings_footer() {
	$postratings_max = intval(get_option('postratings_max'));
	$postratings_custom = intval(get_option('postratings_customrating'));
	echo "\n".'<!-- Start Of Script Generated By WP-PostRatings 1.41 -->'."\n";
	echo '<script type="text/javascript">'."\n";
	echo '/* <![CDATA[ */'."\n";
	if($postratings_custom) {
		for($i = 1; $i <= $postratings_max; $i++) {
			echo "\t".'var ratings_'.$i.'_mouseover_image = new Image();'."\n";
			echo "\t".'ratings_'.$i.'_mouseover_image.src = "'.plugins_url('wp-postratings').'/images/'.get_option('postratings_image').'/rating_'.$i.'_over.'.RATINGS_IMG_EXT.'";'."\n";
		}
	} else {
		echo "\t".'var ratings_mouseover_image = new Image();'."\n";
		echo "\t".'ratings_mouseover_image.src = "'.plugins_url('wp-postratings').'/images/'.get_option('postratings_image').'/rating_over.'.RATINGS_IMG_EXT.'";'."\n";
	}
	echo '/* ]]> */'."\n";
	echo '</script>'."\n";
	wp_print_scripts(array('jquery', 'wp-postratings'));
	echo '<!-- End Of Script Generated By WP-PostRatings 1.41 -->'."\n";
}
...
function ratings_header_admin() {
	wp_register_script('wp-postratings-admin', plugins_url('wp-postratings/postratings-admin-js.js'), false, '1.41');
	wp_register_style('wp-postratings-admin', plugins_url('wp-postratings/postratings-admin-css.css'), false, '1.41', 'all');
	wp_print_styles('wp-postratings-admin');
}

### Function: Displays Ratings Footer In WP-Admin
add_action('admin_footer', 'ratings_footer_admin');
function ratings_footer_admin() {
	echo '<!-- Start Of Script Generated By WP-PostRatings 1.41 -->'."\n";
	...
	wp_print_scripts(array('jquery', 'wp-postratings-admin'));
	echo '<!-- End Of Script Generated By WP-PostRatings 1.41 -->'."\n";
}
