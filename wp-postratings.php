...
function the_ratings($start_tag = 'div', $custom_id = 0, $display = true) {
	...
	if(intval($postratings_ajax_style['loading']) == 1) {
		$loading = "\n<$start_tag id=\"post-ratings-$id-loading\"  class=\"post-ratings-loading\"><img src=\"".get_option('siteurl')."/wp-content/plugins/wp-postratings/images/loading.gif\" width=\"16\" height=\"16\" alt=\"".__('Loading', 'wp-postratings')." ...\" title=\"".__('Loading', 'wp-postratings')." ...\" class=\"post-ratings-image\" />&nbsp;".__('Loading', 'wp-postratings')." ...</".$start_tag.">\n";
	} else {
		$loading = '';
	}
...
function the_ratings_header() {
	...
	wp_register_script('wp-postratings', '/wp-content/plugins/wp-postratings/postratings-js-packed.js', false, '1.31');
	echo "\n".'<!-- Start Of Script Generated By WP-PostRatings 1.31 -->'."\n";
	echo '<script type="text/javascript">'."\n";
	echo '/* <![CDATA[ */'."\n";
	echo "\t".'var using_wp_cache = '.$using_wp_cache.";\n";
	echo "\t".'var site_url = \''.get_option('siteurl')."';\n";
	echo "\t".'var ratings_ajax_url = \''.get_option('siteurl').'/wp-content/plugins/wp-postratings/wp-postratings.php'."';\n";
	...
	echo "\t".'var ratings_page_hash = \''.$wp_cache_key."';\n";
	if($postratings_custom) {
		for($i = 1; $i <= $postratings_max; $i++) {
			echo "\t".'var ratings_'.$i.'_mouseover_image = new Image();'."\n";
			echo "\t".'ratings_'.$i.'_mouseover_image.src = site_url + "/wp-content/plugins/wp-postratings/images/" + ratings_image + "/rating_'.$i.'_over.gif";'."\n";
		}
	} else {
		echo "\t".'var ratings_mouseover_image = new Image();'."\n";
		echo "\t".'ratings_mouseover_image.src = site_url + "/wp-content/plugins/wp-postratings/images/" + ratings_image + "/rating_over.gif";'."\n";
	}
	...
	if(@file_exists(TEMPLATEPATH.'/postratings-css.css')) {
		...
		echo '<link rel="stylesheet" href="'.get_option('siteurl').'/wp-content/plugins/wp-postratings/postratings-css.css" type="text/css" media="screen" />'."\n";
	}	
...
function ratings_header_admin() {
	wp_register_script('wp-postratings-admin', '/wp-content/plugins/wp-postratings/postratings-admin-js-packed.js', false, '1.31');
	echo "\n".'<!-- Start Of Script Generated By WP-PostRatings 1.31 -->'."\n";
	echo '<script type="text/javascript">'."\n";
	echo '/* <![CDATA[ */'."\n";
	echo "\t".'var postratings_admin_ajax_url = \''.get_option('siteurl').'/wp-content/plugins/wp-postratings/postratings-admin-ajax.php'."';\n";
	echo '/* ]]> */'."\n";
	echo '</script>'."\n";
...
function ratings_images_folder($folder_name) {
	...
	$postratings_path = ABSPATH.'/wp-content/plugins/wp-postratings/images/'.$folder_name;
	$images_count_temp = 1;
	$images_count = 1;
	$count = 0;
	$rating['max'] = 0;
